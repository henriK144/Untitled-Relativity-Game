...
A 2D platformer where the player must switch frames of reference from a train to a camera pointed at the train, and exploit the fixed speed of light to induce time dilation, length contraction, and the relativity of simultaneity, to reach the end of each level. Controls and other information are displayed in-game. */
...
pair<Uint8,Uint8> dopplerShift(float dFactor) 
{
	...
	shiftAmounts.second = -9.96*dFactor + 13.984;
...
int main(int argc, char* args[])
{
	...
	string tutorialText[3] = {"The train you are currently travelling on is a mobile laboratory of Wyman-McCarthy Scientific Technologies. It is currently moving at a speed of 209854720.6 metres per second, and can be viewed externally on one of our relativity cameras which are situated at regular intervals across from the tracks. From the frame of reference of this camera, the effects of special relativity on the train cars become apparent - most notably, time will be dilated and all chronological events will slow down. Feel free to view the footage of the relativity camera using any blue computer situated on the train.", "Another relativistic effect that you may have noticed while on board the train is that of length contraction, whereby an object in motion has its length compressed. This is, much like time dilation, a mathematical consequence of the fact that the speed of light remains constant in all reference frames. Moving fast enough to induce such a contraction may aid you in reaching certain areas of the train.", "One final example of relativistic effects in a frame of reference is the relativity of simultaneity, through which events that occur at the same time in one frame may not be simultaneous in other frames. Our newly-patented Simultaneity Cameras are positioned so as to best perceive this effect, and can be viewed from the green laptops on the train."};
	...
	SDL_Texture* spriteArray[6] = {player, chalkboard, spaceBackground, nullSprite, windowBackground, indoorBackground}; // Packaging textures into arrays will help with organization.
	...
	const Entity implicitCeiling(0, 0, WINDOW_WIDTH, 5, window.loadTexture("res/gfx/miscellaneous/BLANK_ICON.png"));
	...
	//musicVolume = 0; soundVolume /= 4; // To make it a bit quieter during testing.
	...
	float gamma = lorentzFactor(train, camera); // Currently 1.25
						...
						case SDL_KEYDOWN: // Handles discrete key presses.
							...
							switch(event.key.keysym.sym)
							{
		                        ...
		                        case SDLK_q:
		                        	...
		                        	break; // Can be modified with a brief animation for the quit to title implementation.
		                    	default:
		                        	break;
		                	}
